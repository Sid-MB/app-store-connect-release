...
const WORKER_CONFIG = resolve(dirname(fileURLToPath(import.meta.url)), "../../worker/wrangler.jsonc");
const REQUIRED_WORKER_SECRETS = ["APPLE_WEBHOOK_SECRET", "GITHUB_DISPATCH_TOKEN"];
...
/** Encode a JWT segment using unpadded base64url. */
function encodeBase64Url(value) {
  return Buffer.from(typeof value === "string" ? value : JSON.stringify(value)).toString("base64url");
}

/** Generate a 19-minute App Store Connect API JWT for a team or individual key. */
...
/** Expand a user-entered private key path without invoking a shell. */
...
/** Inspect an existing Worker and report configured secret names without reading their write-only values. */
...
/** Deploy the packaged Worker code and configuration while preserving any secrets already stored by Cloudflare. */
...
/** Confirm the public Worker endpoint is reachable and is configured for the repository being managed. */
...
/** Generate the caller workflow committed into every target repository. */
...
/** Commit the caller workflow through GitHub's contents API so repository_dispatch can use it immediately from the default branch. */
...
/** Parse Wrangler's documented NDJSON deployment output and return its public workers.dev target. */
function readWorkerUrl(outputPath, fallbackOutput) {
  const records = existsSync(outputPath)
...
/** Create or rotate the matching App Store Connect webhook without producing duplicates on repeated setup runs. */
...
/** Run the complete, repeatable setup for the repository containing the user's current working directory. */
async function setup() {
  ...
  const appId = await select({
    ...
    choices: appsResponse.data.map((app) => ({ name: `${app.attributes.name} (${app.attributes.bundleId})`, value: app.id }))
  });
